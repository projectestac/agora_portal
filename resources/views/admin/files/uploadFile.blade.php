<div id="uploadField">
    ...
    <small class="text-muted">{{ __('file.upload_max_size', ['size' => $maxFileSize]) }} - {{ __('file.allowed_extensions', ['extensions' => $ext]) }}.</small>
    <form id="uploadFileForm" class="hidden" method="post" enctype="multipart/form-data"
        action="{{ route('files.upload', base64_encode($directory)) }}" style="padding: 20px;">
        @csrf
