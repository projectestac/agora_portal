<?php

return [
    'batch_actions' => 'Accions massives',
    'queries' => 'Consultes',
    'query_execution' => 'Executa una consulta',
    'query_execution_confirm' => 'Confirma l\'execució de la consulta',
    'query_to_execute' => 'Consulta a executar',
    'query_executed' => 'Consulta executada',
    'client_selection' => 'Selecció de clients',
    'all_clients' => 'Tots els clients',
    'only_selected' => 'Només els seleccionats',
    'select_file_clients' => 'Selecciona amb un fitxer de clients',
    'deselect_all' => 'Desselecciona-ho tot',
    'file_header_check' => 'La capçalera del fitxer CSV ha de ser \'db_id\', \'code\' o \'dns\'',
    'selected' => 'S\'han seleccionat',
    'clients' => 'client/s',
    'order_by' => 'Ordena per',
    'search_engine' => 'Cercador',
    'sql_query' => 'Consulta SQL',
    'save_query' => 'Desa la consulta',
    'saved_queries' => 'Consultes desades',
    'search_queries' => 'Escriviu per cercar consultes...',
    'update_query' => 'Actualitza la consulta',
    'run_query' => 'Executa la consulta',
    'modify_query' => 'Modifica la consulta',
    'add_description' => 'Escriu una descripció',
    'query_type' => 'Tipus de consulta',
    'query_added' => 'La consulta s\'ha afegit correctament',
    'query_updated' => 'La consulta s\'ha actualitzat correctament',
    'query_removal_confirm' => 'Confirmeu que voleu esborrar aquesta consulta?',
    'query_deleted' => 'La consulta s\'ha esborrat correctament',
    'query_syntax' => 'Sintaxi de les consultes',
    'query_type_select' => 'SELECT',
    'query_type_insert' => 'INSERT',
    'query_type_update' => 'UPDATE',
    'query_type_delete' => 'DELETE',
    'query_type_alter' => 'ALTER',
    'query_type_drop' => 'DROP',
    'query_type_other' => 'ALTRES',
    'instances_to_exec' => 'Instàncies on s\'executarà la consulta',
    'all' => 'Totes',
    'execution_summary' => 'Resum de l\'execució (Dades agregades)',
    'execution_results' => 'Resultats de l\'execució (Una fila per instància)',
    'result_or_num_results' => 'Resultat o nombre de resultats',
    'result' => 'Resultat',
    'num_ocurrences' => 'Nombre d\'ocurrències',
    'result_preview' => 'Vista prèvia del resultat',
    'num_results' => 'Nombre de resultats',
    'num_records' => 'Nombre de registres',
    'instances_count' => 'Nombre d’instances',
    'detailed_results' => 'Resultats detallats',
    'show_all' => 'Mostra-ho tot',
    'hide_all' => 'Amaga-ho tot',
    'affected_rows' => 'Files afectades',
    'execution_error' => 'Error d\'execució',
    'execution_success' => 'Execució exitosa',
    'operation' => 'Operació',
    'operations' => 'Operacions',
    'url_has_not_returned_actions' => 'L\'URL :url no ha retornat cap acció',
    'action' => 'Acció',
    'priority' => 'Prioritat',
    'low' => 'Baixa',
    'medium' => 'Mitjana',
    'high' => 'Alta',
    'highest' => 'La més alta',
    'send_to_queue' => 'Envia a la cua',
    'confirm_execution' => 'Confirma l\'execució',
    'confirm_run_action_again' => 'Confirmeu que voleu tornar a executar aquesta acció?',
    'no_params' => 'Sense paràmetres',
    'operation_to_queue' => 'Operació a afegir a la cua',
    'instances_on_execute' => 'Instàncies on s\'executarà l\'operació',
    'operation_queued' => 'L\'operació s\'ha afegit a la cua correctament',
    'queue' => 'Cua',
    'queues' => 'Cues',
    'queues_operations' => 'Cues d\'operacions',
    'queued_at' => 'Afegida a la cua el',
    'queue_pending' => 'Pendent',
    'queue_success' => 'Executades amb èxit',
    'queue_fail' => 'Executades amb errors',
    'batch_creation' => 'Creació massiva',
    'execution_log' => 'Registre de l\'execució',
    'params' => 'Paràmetres de l\'execució',
    'execution_error' => 'Error d\'execució',
    'run_again' => 'Torna a executar',
    'remove_queue_operation' => 'Esborra l\'operació de la cua',
    'remove_queue_operation_confirm' => 'Confirmeu que voleu esborrar aquesta operació de la cua?',
    'execute_queue_operation' => 'Executa l\'operació de la cua',
    'execute_queue_operation_confirm' => 'Confirmeu que voleu executar aquesta operació de la cua?',
    'queue_operation_executed' => 'L\'operació de la cua s\'ha executat correctament',
    'queue_operation_not_found' => 'No s\'ha trobat l\'operació de la cua',
    'queue_execution_failed' => 'No s\'ha pogut executar l\'operació de la cua',
    'queue_operation_removed' => 'S\'ha esborrat l\'operació de la cua correctament',
    'queue_operation_not_removed' => 'No s\'ha pogut esborrar l\'operació de la cua',
    'no_operation_pending' => 'No hi ha cap operació pendent',
    'no_action' => 'No s\'ha pogut recuperar la llista d\'accions del servei',
    'batch_creation_instances' => 'Creació massiva d\'instàncies',
    'code_and_server' => 'Codis i servidors',
    'code_and_server_info' => 'Indiqueu el codi i el servidor de les instàncies que voleu crear. El format ha de ser del tipus següent: <codi1>:<servidor1>,<codi2>:<servidor2>,...<codiN>:<servidorN>',
    'automatic_signup' => 'Alta automàtica',
    'instance_created' => 'S\'ha creat la instància per al client <strong>:code</strong>. La contrasenya d\'admin és: <strong>:password</strong>',
    'invalid_code' => 'El codi <strong>:code</strong> no és vàlid',
    'client_not_found' => 'No s\'ha trobat cap client amb codi <strong>:code</strong>',
    'instance_already_exists' => 'Ja existeix una instància per al client amb codi <strong>:code</strong>',
];
